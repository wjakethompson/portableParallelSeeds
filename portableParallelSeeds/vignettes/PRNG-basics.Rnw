...
\maketitle
\begin{abstract}
While working on the R package portableParallelSeeds, I realized that
there are a lot of little details about pseudo random generators that
...
set.seed(12345)
.Random.seed
(MASS::mvrnorm(n=5, mu=c(0,0,0), Sigma = diag(3)))
set.seed(12345)
.Random.seed
(MASS::mvrnorm(n=5, mu=c(0,0,0), Sigma = diag(3)))
set.seed(12345)
...
.Random.seed
rnorm(1)
Y4 <- portableParallelSeeds::mvrnorm(n=10, mu = c(0,0,0), Sigma = diag(3))
Y4
set.seed(12345)
.Random.seed
rnorm(1)
Y5 <- portableParallelSeeds::mvrnorm(n=100, mu = c(0,0,0), Sigma = diag(3))
Y5
@
